...
execute as @s[type=player,tag=nofreeze] run detect ~~2~ barrier 0 setblock ~~2~ air
execute as @s[tag=nofreeze] run tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" is no longer frozen."}]}
...
execute as @s[type=player,tag=!nofreeze] run detect ~~2~ air -1 setblock ~~2~ barrier
tag @s[tag=!nofreeze] add freeze
execute as @s[tag=freeze,tag=!nofreeze] run tellraw @a[tag=op] {"rawtext":[{"text":"§r§6[§aScythe§6]§r "},{"selector":"@s"},{"text":" has been frozen"}]}
